...
// ─── StaticHeadingKI – Mobile ─────────────────────────────────────────────────
function StaticHeadingKIMobile() {
  const { language } = useLanguage()
  const de = language === 'de'
  const { disp: d1, scramble: s1 } = useScramble(de ? 'Generative' : 'Generative')
  const { disp: d2, scramble: s2 } = useScramble(de ? 'Intelligenz' : 'Intelligence')
...
// ─── LoopingPixelImages (Desktop only) ───────────────────────────────────────
const SLOT_POSITIONS = [
  { top: '8%',  left: '48%' }, { top: '5%',  left: '72%' },
  { top: '30%', left: '55%' }, { top: '28%', left: '80%' },
  { top: '55%', left: '48%' }, { top: '58%', left: '74%' },
  { top: '15%', left: '62%' }, { top: '45%', left: '64%' },
]
const IMG_SIZES = [{ w:260 },{ w:300 },{ w:220 },{ w:340 },{ w:200 },{ w:280 },{ w:320 },{ w:240 }]
const SRCS = [
  '/ai-images/nikita-01.jpg','/ai-images/nikita-04.jpg','/ai-images/nikita-05.jpg','/ai-images/nikita-06.jpg',
  '/ai-images/nikita-07.jpg','/ai-images/nikita-08.jpg','/ai-images/nikita-09.jpg','/ai-images/nikita-10.jpg',
  '/ai-images/nikita-11.jpg','/ai-images/nikita-12.jpg','/ai-images/nikita-13.jpg','/ai-images/nikita-14.jpg',
]
type SlotState = 'hidden'|'appearing'|'visible'|'disappearing'
interface Slot { id:number; src:string; pos:{top:string;left:string}; w:number; state:SlotState; _posIdx:number }
...
function LoopingPixelImages() {
  ...
  const spawnImage = useCallback(() => {
    ...
    setSlots(prev => [...prev, {
      id, src: SRCS[Math.floor(Math.random()*SRCS.length)],
      pos: SLOT_POSITIONS[posIdx], w: IMG_SIZES[Math.floor(Math.random()*IMG_SIZES.length)].w,
      state: 'appearing', _posIdx: posIdx,
    }])
  ...
  useEffect(() => {
    const t1=setTimeout(()=>spawnImage(),200); const t2=setTimeout(()=>spawnImage(),900); const t3=setTimeout(()=>spawnImage(),1800)
...
function PixelImageSlot({ slot }: { slot: Slot }) {
  ...
  const pixelRef  = useRef(48); const blurRef = useRef(8)
  const stateRef  = useRef(slot.state); const hovRef = useRef(false)
  useEffect(() => { stateRef.current = slot.state }, [slot.state])
  useEffect(() => { hovRef.current   = hovered     }, [hovered])
  useEffect(() => {
    pixelRef.current = 48; blurRef.current = 8
    const image = new Image(); image.src = slot.src
    const canvas = canvasRef.current; if (!canvas) return
    const ctx = canvas.getContext('2d'); if (!ctx) return
    const onLoad = () => { const ratio = image.naturalHeight/image.naturalWidth; canvas.width=slot.w*2; canvas.height=Math.round(slot.w*2*ratio) }
    image.addEventListener('load', onLoad); if (image.complete) onLoad()
    const animate = () => {
      rafRef.current = requestAnimationFrame(animate)
      const s=stateRef.current; const hov=hovRef.current
      const tp=(hov||s==='visible'||s==='appearing')?1:48; const tb=(hov||s==='appearing'||s==='visible')?0:8
      pixelRef.current+=(tp-pixelRef.current)*0.05; blurRef.current+=(tb-blurRef.current)*0.06
      const ps=Math.max(1,Math.round(pixelRef.current)); const bl=Math.max(0,blurRef.current)
      ...
      if (ps<=2){ctx.filter=bl>0.3?`blur(${bl.toFixed(1)}px)`:'none';ctx.drawImage(image,0,0,cw,ch);ctx.filter='none'}
      else{const sw=Math.max(1,Math.floor(cw/ps));const sh=Math.max(1,Math.floor(ch/ps));ctx.drawImage(image,0,0,sw,sh);ctx.filter=bl>0.3?`blur(${bl.toFixed(1)}px)`:'none';ctx.drawImage(canvas,0,0,sw,sh,0,0,cw,ch);ctx.filter='none'}
    }
    rafRef.current = requestAnimationFrame(animate)
  ...
  return (
    <div onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} style={{
      position:'absolute',top:slot.pos.top,left:slot.pos.left,width:slot.w,height:'auto',
      overflow:'hidden',cursor:'crosshair',zIndex:2,
      transform:hovered?'scale(1.14)':'scale(1)',opacity,
      ...
      <img ref={imgRef} src={slot.src} alt="" style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'fill',display:'block',opacity:0}}/>
    </div>
  )
}
...
function TagPill({ label }: { label: string }) {
  ...
  return (
    <span
      ...
      style={{
        backgroundColor:'#0a0a0a',color:'#ffffff',border:'1px solid rgba(255,255,255,0.35)',
        fontSize:9,letterSpacing:'0.12em',textTransform:'uppercase',padding:'5px 10px',
        transform:pressed?'scale(0.92)':hovered?'scale(0.97)':'scale(1)',
        transition:'transform 0.12s ease',cursor:'default',display:'inline-block',
      }}
    >{label}</span>
  )
}
...
function TextRow({ label, body, tags }: { label: string; body: string; tags: string[] }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:'clamp(8px,0.8vw,12px)'}}>
      <div onMouseEnter={()=>runScramble(label,setLabelDisp,labelRef)} style={{
        fontSize:'clamp(11px,2.2vw,24px)',fontWeight:900,letterSpacing:'-0.5px',
        lineHeight:1.1,textTransform:'uppercase',color:'#ffffff',cursor:'default',userSelect:'none',
      }}>{labelDisp}</div>
      <p style={{color:'rgba(255,255,255,0.65)',fontSize:'clamp(12px,1.2vw,15px)',lineHeight:1.8,fontWeight:400,margin:0}}>{body}</p>
...
// ─── TextRow – Mobile ─────────────────────────────────────────────────────────
function TextRowMobile({ label, body, tags }: { label: string; body: string; tags: string[] }) {
  const labelRef = useRef<ReturnType<typeof setInterval>|null>(null)
  const [labelDisp, setLabelDisp] = useState(label)
  useEffect(() => { runScramble(label, setLabelDisp, labelRef) }, [label])
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      <div onMouseEnter={()=>runScramble(label,setLabelDisp,labelRef)} style={{
        fontSize:'clamp(18px,5.5vw,26px)',fontWeight:900,letterSpacing:'-0.5px',
        lineHeight:1.1,textTransform:'uppercase',color:'#ffffff',cursor:'default',userSelect:'none',
      }}>{labelDisp}</div>
      <p style={{color:'rgba(255,255,255,0.72)',fontSize:'clamp(14px,4vw,17px)',lineHeight:1.75,fontWeight:400,margin:0}}>{body}</p>
      {tags.length > 0 && (
...
export function AISection() {
  ...
  useEffect(() => {
    const fn = () => {
      ...
      const exitStart = vh * 0.6; const exitEnd = vh * 1.6
      ...
      <div ref={outerRef} style={{ position:'relative', zIndex:3, height:'260vh' }}>
        <section id="ki" style={{
          ...
          <div style={{
            ...
            <StaticHeadingKIMobile />

            <div style={{ display:'flex', flexDirection:'column', gap:'clamp(28px,7vw,44px)' }}>
              {T.blocks[lang].map((block, i) => (
                <TextRowMobile key={i} label={block.label} body={block.body} tags={block.tags} />
              ))}
            </div>
          </div>
